...
- Distribuição por Qualis no dashboard só funciona depois que um admin carrega a base CAPES (ISSN → estrato) na aba "Base Qualis (CAPES)" — o cruzamento por ISSN já roda automaticamente na importação (`LattesImporter::buscarQualisPorIssn`), mas produções importadas antes de carregar a base precisam ser reimportadas para ganhar o campo retroativamente
